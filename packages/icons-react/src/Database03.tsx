import * as React from 'react';
import type { SVGProps } from 'react';
import { Ref, forwardRef, memo } from 'react';
const Database03 = (
  props: SVGProps<SVGSVGElement>,
  ref: Ref<SVGSVGElement>
) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    name="database-03"
    ref={ref}
    {...props}
  >
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth={2}
      d="M12.9 19.2c0 .9941-.8059 1.8-1.8 1.8s-1.8-.8059-1.8-1.8m3.6 0c0-.9941-.8059-1.8-1.8-1.8m1.8 1.8h6.3m-9.9 0c0-.9941.8059-1.8 1.8-1.8m-1.8 1.8H3m8.1-1.8v-3.6m8.1-8.1c0 1.4912-3.6265 2.7-8.1 2.7S3 7.1912 3 5.7m16.2 0c0-1.4912-3.6265-2.7-8.1-2.7S3 4.2088 3 5.7m16.2 0v5.4c0 1.494-3.6 2.7-8.1 2.7M3 5.7v5.4c0 1.494 3.6 2.7 8.1 2.7"
    />
  </svg>
);
const ForwardRef = forwardRef(Database03);
const Memo = memo(ForwardRef);
export default Memo;
